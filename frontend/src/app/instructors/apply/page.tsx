...
export default function InstructorApplicationPage() {
  ...
  useEffect(() => {
    ...
    instructorsAPI.getMyProfile()
      .then((res: any) => {
        if (!mounted) return
        // If API returns a profile, consider the user already an instructor (or has an application)
        if (res && res.data) {
          setExistingInstructor(res.data)
        }
      })
      .catch(() => {
        // No profile or unauthorized - ignore silently (user may not be instructor yet)
      })
