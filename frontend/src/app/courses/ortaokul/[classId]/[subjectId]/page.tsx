...
export default function OrtaokulSubjectDetailPage() {
  ...
  const subjectGradients: Record<string, string> = {
    'turkce': 'from-amber-500 to-orange-500',
    'matematik': 'from-blue-500 to-cyan-500',
    'fen-bilimleri': 'from-green-500 to-emerald-500',
    'sosyal-bilgiler': 'from-purple-500 to-pink-500',
    'ingilizce': 'from-sky-500 to-blue-500',
    'din-kulturu': 'from-teal-500 to-cyan-500',
    'gorsel-sanatlar': 'from-rose-500 to-red-500',
    'muzik': 'from-indigo-500 to-purple-500',
    'beden-egitimi': 'from-orange-500 to-red-500'
  }

  const subjectTitle = subjectTitles[subjectId] || subjectId
  const subjectEmoji = subjectEmojis[subjectId] || '📚'
  const subjectGradient = subjectGradients[subjectId] || 'from-blue-500 to-indigo-500'
  ...
  const checkAccess = async () => {
    try {
      ...
      const courseKey = `ortaokul-${classNumber}-${subjectId}`
          ...
          {contentSections.map((section, index) => {
            ...
            return (
              <Card 
                ...
                onClick={() => hasAccess && router.push(`/courses/ortaokul/sinif-${classNumber}/${subjectId}/${section.id}`)}
                style={{
