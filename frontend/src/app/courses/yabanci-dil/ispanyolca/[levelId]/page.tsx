...
export default function IspanyolcaLevelDetailPage() {
  ...
  const levelGradients: Record<string, string> = {
    'a1': 'from-yellow-400 to-orange-500',
    'a2': 'from-orange-500 to-red-500',
    'b1': 'from-red-400 to-rose-500',
    'b2': 'from-rose-500 to-pink-500',
    'c1': 'from-pink-500 to-purple-500',
    'c2': 'from-purple-500 to-red-600'
  }

  const levelTitle = levelTitles[levelId] || levelId
  const levelEmoji = levelEmojis[levelId] || '📚'
  const levelGradient = levelGradients[levelId] || 'from-yellow-500 to-red-500'
  ...
  const checkAccess = async () => {
    try {
      ...
      const courseKey = `ispanyolca-${levelId}`
  ...
  const handlePayment = async () => {
    try {
      ...
      const paymentData = {
        courseId: `ispanyolca-${levelId}`,
        courseName: `İspanyolca ${levelTitle}`,
        amount: 299
      }
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-30">
        ...
        <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-orange-400 rounded-full mix-blend-multiply filter blur-3xl animate-blob animation-delay-2000"></div>
        <div className="absolute top-1/2 left-1/2 w-96 h-96 bg-red-400 rounded-full mix-blend-multiply filter blur-3xl animate-blob animation-delay-4000"></div>
      </div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="text-center mb-20">
          <div className="inline-flex items-center px-8 py-4 bg-white/90 backdrop-blur-md rounded-full border-2 border-yellow-200/50 shadow-xl mb-8 hover:scale-105 transition-transform duration-300">
            ...
            <span className="text-lg text-gray-700 font-bold tracking-wide">İSPANYOLCA - {levelTitle.toUpperCase()}</span>
          </div>
          ...
          {contentSections.map((section, index) => {
            ...
            return (
              <Card 
                ...
                onClick={() => hasAccess && router.push(`/courses/yabanci-dil/ispanyolca/${levelId}/${section.id}`)}
                style={{
