...
export default function IngilizceSeviyeliDetailPage() {
  ...
  const levelGradient = levelGradients[levelId] || 'from-blue-500 to-indigo-500'
  ...
  const checkAccess = async () => {
    try {
      ...
      const courseKey = `ingilizce-${levelId}`
  ...
  const handlePayment = async () => {
    try {
      ...
      const paymentData = {
        courseId: `ingilizce-${levelId}`,
        courseName: `İngilizce ${levelTitle}`,
        amount: 299
      }
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="text-center mb-20">
          <div className="inline-flex items-center px-8 py-4 bg-white/90 backdrop-blur-md rounded-full border-2 border-blue-200/50 shadow-xl mb-8 hover:scale-105 transition-transform duration-300">
            ...
            <span className="text-lg text-gray-700 font-bold tracking-wide">İNGİLİZCE - {levelTitle.toUpperCase()}</span>
          </div>
          ...
          {contentSections.map((section, index) => {
            ...
            return (
              <Card 
                ...
                onClick={() => hasAccess && router.push(`/courses/yabanci-dil/ingilizce/${levelId}/${section.id}`)}
                style={{
