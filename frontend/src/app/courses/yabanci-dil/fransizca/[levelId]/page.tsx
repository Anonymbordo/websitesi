...
export default function FransizcaLevelDetailPage() {
  ...
  const levelGradients: Record<string, string> = {
    'a1': 'from-blue-400 to-indigo-500',
    'a2': 'from-indigo-500 to-purple-500',
    'b1': 'from-purple-400 to-pink-500',
    'b2': 'from-pink-500 to-rose-500',
    'c1': 'from-rose-500 to-red-500',
    'c2': 'from-red-500 to-purple-600'
  }

  const levelTitle = levelTitles[levelId] || levelId
  const levelEmoji = levelEmojis[levelId] || '📚'
  const levelGradient = levelGradients[levelId] || 'from-blue-500 to-red-500'
  ...
  const checkAccess = async () => {
    try {
      ...
      const courseKey = `fransizca-${levelId}`
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      {/* Animated Background */}
      <div className="absolute inset-0 bg-gradient-to-br from-blue-100 via-purple-50 to-red-100"></div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="text-center mb-20">
          <div className="inline-flex items-center px-8 py-4 bg-white/90 backdrop-blur-md rounded-full border-2 border-blue-200/50 shadow-xl mb-8 hover:scale-105 transition-transform duration-300">
            ...
            <span className="text-lg text-gray-700 font-bold tracking-wide">FRANSIZCA - {levelTitle.toUpperCase()}</span>
          </div>
          ...
          {contentSections.map((section, index) => {
            ...
            return (
              <Card 
                ...
                onClick={() => hasAccess && router.push(`/courses/yabanci-dil/fransizca/${levelId}/${section.id}`)}
                style={{
