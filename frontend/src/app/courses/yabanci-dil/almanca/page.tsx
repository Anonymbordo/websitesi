...
export default function AlmancaPage() {
  ...
  const levels = [
    { id: 'a1', title: 'A-1', name: 'Başlangıç', desc: 'Temel kelime ve ifadeler', color: 'from-red-400 to-yellow-500', emoji: '🌱' },
    { id: 'a2', title: 'A-2', name: 'Temel', desc: 'Günlük konuşmalar', color: 'from-red-500 to-orange-500', emoji: '🌿' },
    { id: 'b1', title: 'B-1', name: 'Orta', desc: 'Bağımsız kullanım', color: 'from-orange-400 to-amber-500', emoji: '🌊' },
    { id: 'b2', title: 'B-2', name: 'Orta Üstü', desc: 'Akıcı iletişim', color: 'from-amber-500 to-yellow-500', emoji: '🌌' },
    { id: 'c1', title: 'C-1', name: 'İleri', desc: 'Etkili ve esnek kullanım', color: 'from-yellow-500 to-red-500', emoji: '⭐' },
    { id: 'c2', title: 'C-2', name: 'Profesyonel', desc: 'Ana dil seviyesi', color: 'from-red-500 to-rose-500', emoji: '👑' }
  ]
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="text-center mb-20">
          <div className="inline-flex items-center px-8 py-4 bg-white/90 backdrop-blur-md rounded-full border-2 border-red-200/50 shadow-xl mb-8 hover:scale-105 transition-transform duration-300">
            ...
            <span className="text-lg text-gray-700 font-bold tracking-wide">ALMANCA</span>
          </div>
          
          <h1 className="text-6xl md:text-7xl font-extrabold mb-8 relative">
            <span className="bg-gradient-to-r from-red-600 via-yellow-600 to-orange-600 bg-clip-text text-transparent drop-shadow-lg">
              Almanca Seviyeler
            </span>
            <div className="absolute -bottom-4 left-1/2 transform -translate-x-1/2 w-40 h-1 bg-gradient-to-r from-red-500 to-yellow-500 rounded-full"></div>
          </h1>
          
          <p className="text-2xl text-gray-700 max-w-3xl mx-auto leading-relaxed font-medium mt-8">
            CEFR standartlarına uygun seviye seçimi yaparak Almanca öğrenmeye başlayın
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-20">
          {levels.map((level, index) => (
            <Card 
              ...
              onClick={() => router.push(`/courses/yabanci-dil/almanca/${level.id}`)}
              style={{
            ...
            >
              ...
              <CardContent className="relative p-8 text-center space-y-4">
                ...
                <div className="pt-2">
                  <Button 
                    onClick={(e) => {
                      e.stopPropagation()
                      router.push(`/courses/yabanci-dil/almanca/${level.id}`)
                    }}
              ...
              <div className="absolute top-4 right-4 w-2 h-2 bg-red-400 rounded-full animate-ping opacity-40"></div>
              <div className="absolute bottom-4 left-4 w-2 h-2 bg-yellow-400 rounded-full animate-pulse opacity-40"></div>
            </Card>
          ))}
        </div>
        ...
        <div className="text-center">
          <Card className="relative bg-gradient-to-br from-red-500 via-yellow-500 to-orange-500 border-0 shadow-3xl rounded-[2.5rem] max-w-4xl mx-auto overflow-hidden">
            {/* Animated Shine */}
