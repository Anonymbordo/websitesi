...
export default function RuscaLevelDetailPage() {
  ...
  const levelGradients: Record<string, string> = {
    'a1': 'from-gray-600 to-red-500',
    'a2': 'from-gray-700 to-red-500',
    'b1': 'from-red-500 to-orange-500',
    'b2': 'from-orange-400 to-amber-500',
    'c1': 'from-amber-500 to-yellow-500',
    'c2': 'from-yellow-500 to-red-500'
  }

  const levelTitle = levelTitles[levelId] || levelId
  const levelEmoji = levelEmojis[levelId] || '📚'
  const levelGradient = levelGradients[levelId] || 'from-gray-700 to-red-500'
  ...
  const handlePayment = async () => {
    try {
      ...
      const paymentData = {
        courseId: `rusca-${levelId}`,
        courseName: `Rusça ${levelTitle}`,
        amount: 299
      }
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="text-center mb-20">
          <div className="inline-flex items-center px-8 py-4 bg-white/90 backdrop-blur-md rounded-full border-2 border-gray-200/50 shadow-xl mb-8 hover:scale-105 transition-transform duration-300">
            ...
            <span className="text-lg text-gray-700 font-bold tracking-wide">RUSÇA - {levelTitle.toUpperCase()}</span>
          </div>
          ...
          {contentSections.map((section, index) => {
            ...
            return (
              <Card 
                ...
                onClick={() => hasAccess && router.push(`/courses/yabanci-dil/rusca/${levelId}/${section.id}`)}
                style={{
